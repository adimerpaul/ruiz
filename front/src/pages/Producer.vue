      ...
      <q-table :rows="producers" title="Control de productores" :rows-per-page-options="[0]" :columns="producerColumns" :filter="producerFilterData">
        <template v-slot:top>
          <div class="row full-width">
            <div class="col-4">
              <q-btn class="full-width" icon="add_circle" color="green" label="Crear Productor" @click="producerDialog=true" :loading="loading"  />
            </div>
            <div class="col-4">
  ...
  <q-dialog v-model="producerUpdateDialog">
    <q-card>
      ...
      <q-form @submit.prevent="producerUpdate">
        <q-card-section>
          <div class="row">
            <div class="col-12"><q-input outlined v-model="producerU.name" label="Nombre completo" required /></div>
            <div class="col-12"><q-input outlined v-model="producerU.email" label="Email" type="email" required/></div>
            <div class="col-12"><q-select outlined v-model="producerU.tipo" label="Tipo de Productor" :options="['ADMINISTRADOR','REGISTRADOR']" required /></div>
            <div class="col-12"><q-input outlined v-model="producerU.fechaLimite" label="Fechalimite de contrato" type="date"/></div>
          </div>
        </q-card-section>
        <q-card-section>
          <div class="row">
...
export default {
  ...
  methods: {
    ...
    imprimirCredencial(producer){
      ...
      doc.addImage(img, 'jpg', 5, 5, 100, 50)
      img.src = 'img/atras.png'
      doc.addImage(img, 'jpg', 105, 5, 100, 50)
      doc.text(producer.nombres, 110, 20);
    ...
    getProductors() {
      this.$api.get('producer').then(res => {
